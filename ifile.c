...
unsigned int create_ifile(enum inode_type_e type) {
...
void seek_ifile(file_desc_t *fd, int offset) {
	...
	if (bloc_of_pos(fd->fds_pos) != bloc_of_pos(old_pos)) {
		...
		if (!vbloc)
			...
			memset(fd->fds_buf, 0, sizeof(struct inode_s));
		else
			/* load the bloc */
...
int writec_ifile(file_desc_t *fd, char c) {
	...
	/* is the buffer full? */
	if (ibloc_of_pos(fd->fds_pos) == sizeof(struct inode_s) - 1) {
		/* write the buffer */
		ibloc = vbloc_of_fbloc(fd->fds_inumber, bloc_of_pos(fd->fds_pos),
				FALSE);
		write_bloc(CURRENT_VOL, ibloc, fd->fds_buf);
		/* read the new buffer */
		ibloc = vbloc_of_fbloc(fd->fds_inumber, bloc_of_pos(fd->fds_pos + 1),
				FALSE);
		if (!ibloc)
			memset(fd->fds_buf, 0, sizeof(struct inode_s));
		else
